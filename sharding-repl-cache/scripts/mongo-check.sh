...
docker compose -f ./sharding-repl-cache/compose.yaml exec -T mongos_router mongosh --port ${MONGO_ROUTER_PORT} <<EOF
...
docker compose -f ./sharding-repl-cache/compose.yaml exec -T mongos_router mongosh --port ${MONGO_ROUTER_PORT} --quiet <<EOF
use ${MONGO_DATABASE_NAME}
db.${MONGO_COLLECTION_NAME}.countDocuments()
EOF

docker compose -f ./sharding-repl-cache/compose.yaml exec -T shard1-1 mongosh --port ${MONGO_SHARD11_PORT} --quiet <<EOF
use ${MONGO_DATABASE_NAME}
db.${MONGO_COLLECTION_NAME}.countDocuments()
EOF

docker compose -f ./sharding-repl-cache/compose.yaml exec -T shard2-1 mongosh --port ${MONGO_SHARD21_PORT} --quiet <<EOF
use ${MONGO_DATABASE_NAME}
db.${MONGO_COLLECTION_NAME}.countDocuments()
EOF